...
public class MenuOrderDetailRepositoryImpl implements MenuOrderDetailRepository {
    ...
    public MenuOrderDetail save(MenuOrderDetail entity) throws DataAccessException {
        if(entity.getId() == null){
            String query = "INSERT INTO menu_order_detail(menu_order_id, snack_id, quantity, sub_total, created_at, updated_at) " +
                    "values (?, ?, ?, ?, ?, ?,)";
            jdbcTemplate.update(query, entity.getMenuOrder().getId(), entity.getSnack().getId(), entity.getQuantity(),
                     entity.getSubTotal(), LocalDateTime.now(), LocalDateTime.now());
        }
        else{
            String query = "UPDATE menu_order_detail SET name = ?, email = ?, username = ?, password = ?, menu_order_detail_type_id = ?, updated_at = ?" +
                    "WHERE id = ?";
            jdbcTemplate.update(query, entity.getMenuOrder().getId(), entity.getSnack().getId(), entity.getQuantity(),
                    entity.getSubTotal(), LocalDateTime.now(), entity.getId());
        }
        return entity;
    }
